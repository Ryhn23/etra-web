...
export class LAppDelegate {
  ...
  private initializeSubdelegates(): void {
    const live2dWidget = document.querySelector('.live2d-widget') as HTMLElement;
    if (!live2dWidget) {
      console.error('Live2D widget not found');
      return;
    }
    ...
    for (let i = 0; i < LAppDefine.CanvasNum; i++) {
      ...
      // キャンバスを Live2D ウィジェットに追加
      live2dWidget.appendChild(canvas);
    }
